import { Box, CheckCircle2, Clock3, FileCode2, Plus, Star, Trash2 } from 'lucide-react'
...
type Namespace={name:string;team:string;environment:string;cpu:string;memory:string;status:string;created:string;quota:{cpuUsedPct:number;memUsedPct:number;storageUsedPct:number}}
const initial:Namespace[]=[
 {name:'commerce-prod',team:'Commerce',environment:'Produção',cpu:'8 vCPU',memory:'16 GiB',status:'Ativo',created:'12/04/2026',quota:{cpuUsedPct:74,memUsedPct:82,storageUsedPct:55}},
 {name:'data-sandbox',team:'Data Platform',environment:'Sandbox',cpu:'4 vCPU',memory:'8 GiB',status:'Ativo',created:'21/05/2026',quota:{cpuUsedPct:38,memUsedPct:44,storageUsedPct:91}},
 {name:'growth-preview',team:'Growth',environment:'Preview',cpu:'2 vCPU',memory:'4 GiB',status:'Provisionando',created:'Agora',quota:{cpuUsedPct:12,memUsedPct:18,storageUsedPct:9}},
]

const manifestFor = (ns: Namespace) => `apiVersion: v1
kind: Namespace
metadata:
  name: ${ns.name}
  labels:
...
export default function NamespacesPage(){
 ...
 const create=()=>{if(!form.name||!form.team)return;const sizes:Record<string,[string,string]>={Pequeno:['2 vCPU','4 GiB'],Médio:['4 vCPU','8 GiB'],Grande:['8 vCPU','16 GiB']};setItems(v=>[...v,{name:form.name,team:form.team,environment:form.environment,cpu:sizes[form.size][0],memory:sizes[form.size][1],status:'Provisionando',created:'Agora',quota:{cpuUsedPct:5,memUsedPct:8,storageUsedPct:2}}]);setModal(false);setTimeout(()=>setItems(v=>v.map(i=>i.name===form.name?{...i,status:'Ativo'}:i)),2200)}

 const openYaml = (ns: Namespace) => { setYamlTarget(ns); setYamlText(manifestFor(ns)); logAction(t('namespaces.audit.manifestOpened'), ns.name) }
 ...
 return <><PageHeader eyebrow={t('namespaces.eyebrow')} title={t('namespaces.title')} description={t('namespaces.description')} actions={<button className="button button--primary" disabled={!canEdit} title={!canEdit ? 'Ação bloqueada no modo visualizador' : undefined} onClick={()=>setModal(true)}><Plus size={16}/> {t('namespaces.request')}</button>}/>
 ...
 {yamlTarget && <Modal title={`${t('namespaces.manifestTitle')}: ${yamlTarget.name}`} onClose={()=>setYamlTarget(null)}>
  <div className="yaml-editor">
   ...
   <div className="form-actions" style={{marginTop:12}}>
    ...
    <button className="button button--primary" disabled={!canEdit} title={!canEdit ? 'Ação bloqueada no modo visualizador' : undefined} onClick={saveYaml}>{t('namespaces.validateSyntax')}</button>
   </div>
  </div>
 </Modal>}
 </>
